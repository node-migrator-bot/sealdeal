(function() {
  var build, commentDelimiters, compileCSS, compileCSSFile, compileCoffeeFileTo, compileFile, compileHTML, compileHTMLFile, compileJS, compileJSDir, compileJSFile, compileText, concatCSSDir, concatFiles, concatJSDir, copyTree, cssExtensions, cssFilter, cssRoute, extensionRegex, extractJSRequires, extractRequires, fileConfig, fileType, fs, getCompiler, getFiles, htmlExtensions, htmlFilter, isInsideDir, itemToLast, jsExtensions, jsFilter, jsRoute, mkdirRecursive, path, preprocessorRoute, readFile, readHTMLPage, regexFilter, removeExt, requireCompiler, walk, withoutItem, writeFileRecursive,
  ...
  build = function(config) {
    var concatCSS, concatJS, concatToTarget, cssDirs, cssDirsPath, data, fileDir, filename, files, jsDirs, jsDirsPath, layoutPath, relativeFilename, src, target, targetFilename, templatePath, _i, _len;
    src = config.src;
  ...
  jsRoute = function(appPath, config) {
    return function(req, res) {
      res.contentType('application/javascript');
      return res.send(concatJSDir(path.join(appPath, config.concatJS)));
    };
  };

  cssRoute = function(appPath, config) {
    return function(req, res) {
      res.contentType('text/css');
      return res.send(concatCSSDir(path.join(appPath, config.concatCSS)));
    };
  };
